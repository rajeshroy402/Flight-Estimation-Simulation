...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Mat3x3F rotation_mat = Mat3x3F::Zeros();
  rotation_mat(0, 0) = 1;
  rotation_mat(0, 1) = sin(rollEst) * tan(pitchEst);
  rotation_mat(0, 2) = cos(rollEst) * tan(pitchEst);
  rotation_mat(1, 0) = 0;
  rotation_mat(1, 1) = cos(rollEst);
  rotation_mat(1, 2) = -sin(rollEst);
  rotation_mat(2, 0) = 0;
  rotation_mat(2, 1) = sin(rollEst) * 1 / cos(pitchEst);
  rotation_mat(2, 2) = cos(rollEst) * 1 / cos(pitchEst);

  V3F Euler_dot = rotation_mat * gyro;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(1, 0) = cos(pitch) * cos(roll);
